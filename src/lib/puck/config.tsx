...
// import { DropZone, registerOverlayPortal, FieldLabel, createUsePuck } from "@measured/puck";
...
// import { ActionStateProvider, useActionState, runActions } from "./actions";
...
// /**
//  * Puck configuration defining categories and components. This configuration
//  * embraces many advanced patterns recommended by Puck to handle flexible layouts,
//  * including flex and grid containers with dedicated item components, multi‑column
//  * layouts, and array fields for dynamic lists.
//  *
//  * References:
//  * – Flex container – Flex item pattern showing how to structure a flex container
//  *   and allow only FlexItem children with controls for grow/shrink/basis【579873606476094†L667-L786】.
//  * – Grid container – Grid item pattern with span controls for rows and columns【579873606476094†L196-L318】.
//  */
// export const config = {
//   categories: {
//     layout: {
...
//       ],
//     },
//     // Typography category includes standard heading and paragraph components as well
//     // as a RichText component for HTML content. RichText allows editors to
//     // enter raw HTML snippets to achieve more complex formatting. Editors
//     // should be mindful of XSS concerns when using this field; it will be
//     // injected via dangerouslySetInnerHTML into the page. This addition was
//     // inspired by community plugins highlighted in the awesome‑puck repository
//     // which showcase rich‑text editors【62556322094410†L6-L31】.
//     typography: { title: "Typography", components: ["Heading", "Text", "RichText"] },
//     actions: { title: "Actions", components: ["Button"] },
//     media: { title: "Media", components: ["Image", "Video", "Gallery"] },
//     custom: { title: "Custom", components: ["Testimonials", "LinksList", "Logos"] },
//     // Navigation components provide site‑wide navigation structures such as a
//     // collapsible navigation bar or side navigation. They leverage responsive
//     // JavaScript to collapse at breakpoints and optionally integrate with
//     // ActionState flags for programmatic control.
//     navigation: { title: "Navigation", components: ["Navbar", "Sidebar"] },
//     // Overlay components live above the page and typically hide the underlying
//     // content. The Modal component shows a dialog controlled by a flag and
//     // supports primary/secondary actions.
//     overlays: { title: "Overlays", components: ["Modal"] },
//     // Widgets category includes small self‑contained components such as QR codes,
//     // Spotify embeds, animated text and coloured boxes. These components can
//     // often enhance pages with additional interactive content.
//     widgets: {
//       title: "Widgets",
//       components: [
//         "TypingText",
...
//         "Card",
//         "LinksList",
//         // New widget components for extended control and editor information
//         "ColorPickerBox",
//         "SelectedInfo",
//       ],
//     },
//     // Data category holds components that fetch and display remote data or
...
//     // Structure category contains components that organise content into
//     // collapsible panels. The Accordion component allows users to toggle the
//     // visibility of sections of content, reducing clutter and helping
//     // maintain focus【243256177162445†L136-L142】.
//     structure: { title: "Structure", components: ["Accordion", "Tabs"] },

//     // Inputs category holds interactive form elements such as switches and
//     // sliders. Switches toggle the state of a setting on or off【720273760266879†L37-L42】,
//     // while sliders allow users to make selections from a range of values along a bar【263366092288640†L169-L176】.
//     inputs: { title: "Inputs", components: ["Switch", "Slider", "BeverageSelector"] },
//   },
//   // Root defines the page level fields and wrapper.
//   root: {
//     fields: {
//       title: { type: "text", label: "Page title", defaultValue: "Untitled" },
//       viewport: {
//         type: "select",
//         label: "Viewport",
//         options: [
...
//         editingPadding: { type: "number", label: "Editing extra padding (px)", defaultValue: 8 },
//       },
//       render: ({ children: Content, maxWidth, paddingY, paddingX, background, variant, gradientFrom, gradientTo, minHeight, center, editingPadding, puck }: any) => {
//         const path = getPathFromPuck(puck);
//         const isEditing = isEditingFromPuck(puck);
//         const isSelected = selectionStore.has(path);
...
//           else selectionStore.toggle(path, false);
//         };
//         return <div ref={puck?.dragRef} data-puck-path={path || undefined} style={final} onMouseDown={onMouseDown}>{typeof Content === "function" ? <Content /> : null}</div>;
//       },
//     },
...
//       fields: {
//         children: { type: "slot", label: "Items" },
//         dirMobile: { type: "select", label: "Mobile direction", options: [ { label: "Column", value: "column" }, { label: "Row", value: "row" } ], defaultValue: "column" },
//         dirTablet: { type: "select", label: "Tablet direction", options: [ { label: "Inherit", value: "inherit" }, { label: "Column", value: "column" }, { label: "Row", value: "row" } ], defaultValue: "inherit" },
//         dirDesktop: { type: "select", label: "Desktop direction", options: [ { label: "Inherit", value: "inherit" }, { label: "Column", value: "column" }, { label: "Row", value: "row" } ], defaultValue: "inherit" },
...
//         align: { type: "select", label: "Align items", options: [ { label: "Start", value: "flex-start" }, { label: "Center", value: "center" }, { label: "End", value: "flex-end" }, { label: "Stretch", value: "stretch" } ], defaultValue: "stretch" },
//         justify: { type: "select", label: "Justify", options: [ { label: "Start", value: "flex-start" }, { label: "Center", value: "center" }, { label: "End", value: "flex-end" }, { label: "Between", value: "space-between" }, { label: "Around", value: "space-around" }, { label: "Evenly", value: "space-evenly" } ], defaultValue: "flex-start" },
//         wrap: { type: "select", label: "Wrap", options: [ { label: "No", value: "nowrap" }, { label: "Wrap", value: "wrap" } ], defaultValue: "wrap" },
...
//         editingPadding: { type: "number", label: "Editing extra padding (px)", defaultValue: 8 },
//       },
//       render: ({ children: Content, dirMobile, dirTablet, dirDesktop, gapMobile, gapTablet, gapDesktop, align, justify, wrap, tabletBreakpoint, desktopBreakpoint, editingPadding, puck }: any) => {
//         const path = getPathFromPuck(puck);
//         const isEditing = isEditingFromPuck(puck);
//         const isSelected = selectionStore.has(path);
...
//           else selectionStore.toggle(path, false);
//         };
//         return <div ref={(el) => { (ref as any).current = el; if (puck?.dragRef) (puck.dragRef as any)(el); }} data-puck-path={path || undefined} style={final} onMouseDown={onMouseDown}>{typeof Content === "function" ? <Content /> : null}</div>;
//       },
//     },
...
//         editingPadding: { type: "number", label: "Editing extra padding (px)", defaultValue: 8 },
//       },
//       render: ({ children: Content, layout, colsMobile, colsTablet, colsDesktop, gapMobile, gapTablet, gapDesktop, autoRows, tabletBreakpoint, desktopBreakpoint, editingPadding, puck }: any) => {
//         const path = getPathFromPuck(puck);
//         const isEditing = isEditingFromPuck(puck);
//         const isSelected = selectionStore.has(path);
//         const { ref, width } = useElementWidth<HTMLDivElement>();
...
//         align: { type: "select", label: "Align", options: [ { label: "Left", value: "left" }, { label: "Center", value: "center" }, { label: "Right", value: "right" } ], defaultValue: "left" },
//         gap: { type: "number", label: "Gap (px)", defaultValue: 24 },
//         paddingY: { type: "number", label: "Padding Y (px)", defaultValue: 48 },
//         paddingX: { type: "number", label: "Padding X (px)", defaultValue: 24 },
//         editingPadding: { type: "number", label: "Editing extra padding (px)", defaultValue: 8 },
//       },
//       render: ({ columns, copyright, year, background, textColor, align, gap, paddingY, paddingX, editingPadding, puck }: any) => {
//         const path = getPathFromPuck(puck);
//         const isEditing = isEditingFromPuck(puck);
//         const isSelected = selectionStore.has(path);
...
//                       <a key={j} href={l?.href || "#"} style={{ color: textColor || undefined, textDecoration: "none", opacity: 0.85 }} onClick={(e) => isEditing ? e.preventDefault() : undefined}>{l?.label || ""}</a>
//                     )) : null}
//                   </div>
//                 </div>
//               ))}
//             </div>
//             <div style={{ fontSize: 12, opacity: 0.7, textAlign: align as any }}>{copyright || ""} {year}</div>
//           </footer>
//         );
//       },
//     },
...
//         },
//       },
//       // Dynamically expose placement fields when inside Grid or FlexContainer
//       resolveFields: (data: any, ctx: any) => {
//         const parent = ctx?.parent;
//         const isGrid = parent?.type === 'Grid' || parent?.type === 'ResponsiveGrid';
//         const isFlex = parent?.type === 'FlexContainer' || parent?.type === 'ResponsiveFlex' || parent?.type === 'Stack';
//         const extra: any = {};
//         if (isGrid) {
//           extra.columns = { type: 'number', label: 'Grid column span', defaultValue: 1 };
//           extra.rows = { type: 'number', label: 'Grid row span', defaultValue: 1 };
//         }
//         if (isFlex) {
//           extra.flexGrow = { type: 'number', label: 'Flex grow', defaultValue: 0 };
//           extra.flexShrink = { type: 'number', label: 'Flex shrink', defaultValue: 1 };
//           extra.flexBasis = { type: 'text', label: 'Flex basis', defaultValue: 'auto' };
//         }
//         return { ...data.fields, ...extra };
//       },
//       render: ({ children: Content, columns, rows, flexGrow, flexShrink, flexBasis, debugOverlay, puck }: any) => {
//         const path = getPathFromPuck(puck);
//         const isEditing = isEditingFromPuck(puck);
//         const isSelected = selectionStore.has(path);
//         // Determine context by inspecting parent type if available
...
//           flexShrink: inFlex && typeof flexShrink === 'number' ? flexShrink : undefined,
//           flexBasis: inFlex && flexBasis ? flexBasis : undefined,
//           position: 'relative',
...
//             {typeof Content === 'function' ? <Content /> : null}
//             {isEditing && debugOverlay === 'true' ? (
//               <span style={{ position: 'absolute', top: 4, left: 6, fontSize: 11, fontFamily: 'monospace', background: 'rgba(0,0,0,0.6)', color: '#fff', padding: '2px 4px', borderRadius: 4, pointerEvents: 'none' }}>
//                 {inGrid ? `c${columns || 1} r${rows || 1}` : inFlex ? `g${flexGrow || 0} s${flexShrink || 1} b${flexBasis || 'auto'}` : 'item'}
//               </span>
//             ) : null}
//           </div>
//         );
...
//         orientation: { type: 'select', label: 'Orientation', options: [ { label: 'Vertical', value: 'vertical' }, { label: 'Horizontal', value: 'horizontal' } ], defaultValue: 'vertical' },
//         tabletBreakpoint: { type: 'number', label: 'Tablet ≥ px', defaultValue: 640 },
//         desktopBreakpoint: { type: 'number', label: 'Desktop ≥ px', defaultValue: 1024 },
//         showOutline: { type: 'select', label: 'Show outline (editing)', options: [ { label: 'No', value: 'false' }, { label: 'Yes', value: 'true' } ], defaultValue: 'true' },
//       },
//       render: ({ sizeMobile, sizeTablet, sizeDesktop, orientation, tabletBreakpoint, desktopBreakpoint, showOutline, puck }: any) => {
//         const { ref, width } = useElementWidth<HTMLDivElement>();
//         const tabletBp = typeof tabletBreakpoint === 'number' ? tabletBreakpoint : 640;
//         const desktopBp = typeof desktopBreakpoint === 'number' ? desktopBreakpoint : 1024;
...
//         }
//         return <div ref={(el) => { (ref as any).current = el; if (puck?.dragRef) (puck.dragRef as any)(el); }} style={style} />;
//       },
//     },
...
//         editingPadding: { type: 'number', label: 'Editing extra padding (px)', defaultValue: 8 },
//       },
//       render: ({ children: Content, minWidthMobile, minWidthTablet, minWidthDesktop, gapMobile, gapTablet, gapDesktop, tabletBreakpoint, desktopBreakpoint, editingPadding, puck }: any) => {
//         const { ref, width } = useElementWidth<HTMLDivElement>();
//         const tabletBp = typeof tabletBreakpoint === 'number' ? tabletBreakpoint : 640;
//         const desktopBp = typeof desktopBreakpoint === 'number' ? desktopBreakpoint : 1024;
...
//           defaultValue: "flex-start",
//         },
//         // Visual hint sizes for empty drop areas during editing
//         minWidth: { type: "number", label: "Min width (px)", defaultValue: 0 },
//         minHeight: { type: "number", label: "Min height (px)", defaultValue: 64 },
//         editingPadding: { type: "number", label: "Editing extra padding (px)", defaultValue: 8 },
//       },
//       render: ({ gap, wrap, alignItems, justifyContent, minWidth, minHeight, editingPadding, puck }: any) => {
//         const path = getPathFromPuck(puck);
//         const isEditing = isEditingFromPuck(puck);
//         const isSelected = selectionStore.has(path);
//         const extraPad = isEditing && typeof editingPadding === 'number' && editingPadding > 0 ? editingPadding : 0;
...
//             }
//           : {};
//         // When selected, apply an outline. Extra padding only applies during editing.
//         const style = isSelected ? { ...base, ...outlineForSelected, ...hint } : { ...base, ...hint };
//         const onMouseDown = (e: any) => {
//           e.stopPropagation();
//           if (!isEditing) return;
...
//           else selectionStore.toggle(path, false);
//         };
//         // Compute padding when editing to make selection easier.
//         const extraPadding = isEditing ? "8px" : undefined;
//         return (
//           <div
//             ref={puck?.dragRef}
//             data-puck-path={path || undefined}
...
//               zone="flex-zone"
//               allow={["FlexItem"]}
//               /* Provide a minimum height so that empty containers remain clickable and
//                  hint the horizontal axis for drag‑and‑drop. */
//               minEmptyHeight={Math.max(32, typeof minHeight === 'number' ? minHeight : 0)}
//               collisionAxis="x"
//               style={{ display: "contents", padding: extraPadding }}
//             />
//           </div>
//         );
//       },
...
//       fields: {
//         children: { type: "slot", label: "Content" },
//         grow: { type: "number", label: "Flex grow", defaultValue: 1 },
//         shrink: { type: "number", label: "Flex shrink", defaultValue: 1 },
//         basis: { type: "text", label: "Flex basis", defaultValue: "auto" },
//       },
//       render: ({ children: Content, grow, shrink, basis, minWidth, minHeight, puck }: any) => {
//         // Do not wrap in an extra div; use DropZone directly with dragRef.
...
//             minEmptyHeight: Math.max(32, typeof minHeight === 'number' ? minHeight : 0),
//           },
//           typeof Content === "function" ? React.createElement(Content as any) : null
//         );
//       },
//     },
//     /**
...
//         editingPadding: { type: "number", label: "Editing extra padding (px)", defaultValue: 8 },
//       },
//       render: ({ layout, columns, rows, autoRows, gap, minWidth, minHeight, editingPadding, puck }: any) => {
//         // Base grid style: ensure the grid stretches to the full width of its parent
//         const presetMap: Record<string, { cols?: string; rows?: string; autoRows?: string }> = {
//           'hero-split': { cols: 'repeat(2, minmax(0,1fr))', rows: 'auto' },
//           'sidebar-left': { cols: '240px 1fr', rows: 'auto' },
//           'sidebar-right': { cols: '1fr 240px', rows: 'auto' },
//           'feature-cards': { cols: 'repeat(auto-fit, minmax(240px, 1fr))', rows: 'auto' },
//           'gallery': { cols: 'repeat(auto-fill, minmax(160px,1fr))', rows: 'auto' },
//           'bento': { cols: 'repeat(6, 1fr)', rows: 'auto' },
//         };
...
//           gridTemplateRows: preset?.rows || rows || undefined,
//           gridAutoRows: preset?.autoRows || autoRows || undefined,
//           gap: gap ? `${gap}px` : undefined,
//           padding: extraPad ? `${extraPad}px` : undefined,
//         };
//         const path = getPathFromPuck(puck);
...
//           else selectionStore.toggle(path, false);
//         };
//         // Provide padding only when editing and a minimum height when empty.
//         const extraPadding = isEditing ? "8px" : undefined;
//         return (
//           <div
//             ref={puck?.dragRef}
...
//               allow={["GridItem"]}
//               minEmptyHeight={Math.max(32, typeof minHeight === 'number' ? minHeight : 0)}
//               /* For grids, allow Puck to infer axis based on CSS; no collisionAxis set */
//               style={{ display: "contents", padding: extraPadding }}
//             />
//           </div>
//         );
...
//                 <DropZone
//                   zone={`column-${idx}`}
//                   style={{ width: "100%", minHeight: typeof minHeight === 'number' && minHeight > 0 ? `${minHeight}px` : '64px', outline: '1px dashed #e5e7eb', outlineOffset: -4, background: '#fafafa' }}
//                   minEmptyHeight={Math.max(32, typeof minHeight === 'number' ? minHeight : 0)}
//                 />
//               </div>
//             ))}
...
//             ? actions.filter((a: any) => (a?.event || 'click') === eventName)
//             : [];
//           if (acts.length) {
...
//           const clickActs = Array.isArray(actions)
//             ? actions.filter((a: any) => (a?.event || 'click') === 'click')
//             : [];
...
//         const clickActions = Array.isArray(actions)
//           ? actions.filter((a: any) => (a?.event || "click") === "click")
//           : [];
...
//           label: "Subtitle",
//           defaultValue:
//             "A short description of the main value proposition goes here.",
//         },
//         align: {
//           type: "select",
//           label: "Align",
...
//                       <a
//                         href={ctaHref || "#"}
//                         style={btnStyle(String(ctaVariant || "solid"))}
//                         onClick={preventNav}
//                       >
//                         {ctaLabel}
//                       </a>
...
//                       <a
//                         href={cta2Href || "#"}
//                         style={btnStyle(String(cta2Variant || "outline"))}
//                         onClick={preventNav}
//                       >
//                         {cta2Label}
//                       </a>
...
//           getItemSummary: (props: any) =>
//             props?.quote ? `${props.quote.slice(0, 20)}…` : "Testimonial",
//         },
...
//           >
//             <div
//               style={{ maxWidth: "640px", margin: "0 auto", display: "grid", gap: "1.5rem" }}
//             >
...
//           getItemSummary: (props: any) =>
//             props?.src ? props.src.split("/").pop() : "Logo",
//         },
...
//               <a
//                 href={brandHref || "/"}
//                 style={{ display: "inline-flex", alignItems: "center", gap: 8, textDecoration: "none", color: "inherit", fontWeight: 600 }}
//                 onClick={preventNav}
//               >
//                 {brandImageSrc ? (
//                   <img
...
//                     type="button"
//                     onClick={() => setOpen((v) => !v)}
//                     style={{ background: "transparent", border: 0, padding: 8, borderRadius: 6, cursor: "pointer", color: "inherit" }}
//                   >
...
//                         style={{ display: "block", color: "inherit", textDecoration: "none", padding: "0.25rem 0.5rem" }}
//                         onClick={preventNav}
//                       >
//                         {l?.label || "Link"}
//                       </a>
...
//               ) : (
//                 <div
//                   style={{ display: "flex", alignItems: "center", justifyContent: justify, gap: gap ?? 12, marginLeft: 12 }}
//                 >
//                   {(Array.isArray(links) ? links : []).map((l: any, i: number) => {
...
//                         href={l?.href || "#"}
//                         target={l?.target || "_self"}
//                         style={{ color: "inherit", textDecoration: "none", padding: "0.25rem 0.5rem", opacity: active ? 1 : 0.9, fontWeight: active ? 600 : 500 }}
//                         onClick={preventNav}
//                       >
//                         {l?.label || "Link"}
//                       </a>
...
//             onMouseDown={onMouseDown}
//           >
//             <img src={src} alt={url || "QR Code"} width={sz} height={sz} />
//           </div>
//         );
//       },
//     },
...
//               />
//             ) : (
//               <div style={{ width: "100%", height: h, display: "flex", alignItems: "center", justifyContent: "center", background: "#f3f4f6" }}>
...
//           >
//             {image ? (
//               <img src={image} alt={title || ""} style={{ width: 80, height: 80, objectFit: "cover", borderRadius: 4 }} />
//             ) : null}
//             <div style={{ flex: 1 }}>
//               {title ? (
...
//         border: { type: "text", label: "Border", defaultValue: "" },
//       },
//       render: ({ header: HeaderSlot, body: BodySlot, footer: FooterSlot, background, padding, borderRadius, border, puck }: any) => {
//         const path = getPathFromPuck(puck);
//         const isSelected = selectionStore.has(path);
//         const isEditing = isEditingFromPuck(puck);
//         const base: any = {
...
//             onMouseDown={onMouseDown}
//           >
//             {typeof HeaderSlot === "function" ? <div style={{ marginBottom: 8 }}><HeaderSlot /></div> : null}
//             {typeof BodySlot === "function" ? <div style={{ marginBottom: 8 }}><BodySlot /></div> : null}
//             {typeof FooterSlot === "function" ? <div><FooterSlot /></div> : null}
//           </div>
//         );
//       },
//     },
...
//           <div
//             ref={puck?.dragRef}
//             data-puck-path={pathId || undefined}
//             style={style}
//             onMouseDown={onMouseDown}
//           >
...
//         const flagName = String(controlFlag ?? "").trim();
...
//                 type="button"
//                 onClick={() => toggleOpen()}
//                 style={{ position: "fixed", top: 12, [side]: 12, zIndex: 210, padding: 8, borderRadius: 6, border: "1px solid #e5e7eb", background: "#fff", cursor: "pointer" }}
//               >
//                 {isOpen ? "Close" : "Menu"}
//               </button>
//             ) : null}
...
//               type: "select",
//               label: "Target",
//               options: [ { label: "_self", value: "_self" }, { label: "_blank", value: "_blank" } ],
//               defaultValue: "_self",
//             },
//             selector: { type: "text", label: "Selector" },
//             targetElId: { type: "text", label: "Element ID" },
...
//           arrayFields: {
//             event: { type: "select", label: "Event", options: [ { label: "Click", value: "click" } ], defaultValue: "click" },
//             type: {
//               type: "select",
//               label: "Action type",
//               options: [
...
//           await runActions(Array.isArray(primaryActions) ? (primaryActions as any) : [], { isEditing, currentEl: e?.currentTarget, ctxOverride: { flags, setFlag, toggleFlag, allowCustomJS } });
//           if (String(primaryClose || 'true') !== 'false') handleClose();
//         };
//         const handleSecondary = async (e: any) => {
//           await runActions(Array.isArray(secondaryActions) ? (secondaryActions as any) : [], { isEditing, currentEl: e?.currentTarget, ctxOverride: { flags, setFlag, toggleFlag, allowCustomJS } });
...
//                       style={{ padding: '0.5rem 1rem', borderRadius: 4, fontWeight: 600, border: 'none', background: '#111827', color: '#fff', cursor: 'pointer' }}
//                     >
//                       {primaryLabel}
//                     </button>
//                   ) : null}
...
//                       style={{ padding: '0.5rem 1rem', borderRadius: 4, fontWeight: 600, border: '1px solid #111827', background: 'transparent', color: '#111827', cursor: 'pointer' }}
//                     >
//                       {secondaryLabel}
//                     </button>
//                   ) : null}
...
//                   <div
//                     onClick={() => toggle(idx)}
//                     style={{ cursor: 'pointer', padding: '8px 12px', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
//                   >
...
//           label: "Tabs",
//           arrayFields: {
//             title: { type: "text", label: "Tab title", defaultValue: "Tab" },
//             content: { type: "slot", label: "Tab content" },
//           },
...
//         ],
//       },
//       render: ({ defaultTab, tabsAlign, items, puck }: any) => {
...
//         // Register the tab navigation as an overlay portal to make it clickable in edit mode
//         React.useEffect(() => {
//           if (tabsNavRef.current && isEditing) {
//             const cleanup = registerOverlayPortal(tabsNavRef.current);
//             return cleanup;
//           }
...
//             <div 
//               ref={tabsNavRef}
//               style={{ display: 'flex', gap: '4px', borderBottom: '2px solid #e5e7eb', justifyContent: tabsAlign || 'flex-start', flexWrap: 'wrap' }}
//             >
...
//                     key={idx} 
//                     style={{ 
//                       position: isEditing ? 'relative' : (isActive ? 'relative' : 'absolute'),
//                       visibility: isEditing || isActive ? 'visible' : 'hidden',
//                       opacity: isEditing || isActive ? 1 : 0,
//                       pointerEvents: isEditing || isActive ? 'auto' : 'none',
//                       width: '100%',
//                       minHeight: isEditing ? '120px' : undefined,
//                       border: isEditing ? '2px dashed #cbd5e1' : undefined,
//                       borderRadius: isEditing ? '8px' : undefined,
//                       padding: isEditing ? '16px' : undefined,
//                       background: isEditing ? '#f8fafc' : undefined,
//                       marginBottom: isEditing && idx < tabs.length - 1 ? '16px' : undefined,
//                     }}
//                   >
//                     {isEditing && (
//                       <div style={{ 
...
//   LandingPage: {
//     root: {
//       props: { title: "Landing", viewport: "fluid", allowCustomJS: "true", slug: "landing", description: "Landing page template" },
//       children: [
//         { type: "Navbar", props: { brand: "Brand", links: [ { label: "Features", href: "#" }, { label: "Pricing", href: "#" }, { label: "Contact", href: "#" } ] } },
//         { type: "Hero", props: { title: "Build faster with Puck", subtitle: "Drag‑and‑drop responsive layout primitives.", align: "center", background: "#ffffff", overlayOpacity: 0 } },
//         { type: "Section", props: { variant: "muted", center: "true", paddingY: 64, maxWidth: "960px" , children: [
//           { type: "ResponsiveGrid", props: { colsMobile: 1, colsTablet: 2, colsDesktop: 3, gapDesktop: 32, children: [
//             { type: "Card", props: { title: "Flexible", body: "Mix flex, grid, columns." } },
//             { type: "Card", props: { title: "Responsive", body: "Breakpoint‑aware primitives." } },
//             { type: "Card", props: { title: "Composable", body: "Nest any components." } }
//           ] } }
//         ] } },
//         { type: "Section", props: { variant: "gradient", gradientFrom: "#6366f1", gradientTo: "#8b5cf6", center: "true", paddingY: 80, children: [
//           { type: "Heading", props: { level: 2, text: "Get started now" } },
//           { type: "Text", props: { text: "Drop these templates into your app and customise." } },
//           { type: "Button", props: { label: "Start", theme: "brand" } }
//         ] } },
//         { type: "Footer", props: { columns: [ { heading: "Product", links: [ { label: "Features", href: "#" }, { label: "Roadmap", href: "#" } ] }, { heading: "Company", links: [ { label: "About", href: "#" }, { label: "Careers", href: "#" } ] }, { heading: "Legal", links: [ { label: "Privacy", href: "#" }, { label: "Terms", href: "#" } ] } ], copyright: "© Brand", year: new Date().getFullYear() } }
//       ],
//     },
//   },
//   DocsPage: {
//     root: {
//       props: { title: "Docs", viewport: "fluid", allowCustomJS: "true", slug: "docs", description: "Documentation layout" },
//       children: [
//         { type: "Navbar", props: { brand: "Docs", links: [ { label: "Guide", href: "#" }, { label: "API", href: "#" }, { label: "GitHub", href: "#" } ] } },
//         { type: "Section", props: { variant: "plain", paddingY: 48, maxWidth: "1280px", children: [
//           { type: "Columns", props: { distribution: "manual", columns: [ { span: 1 }, { span: 3 } ], gap: 32 } },
//         ] } },
//         { type: "Footer", props: { columns: [ { heading: "Community", links: [ { label: "Forum", href: "#" }, { label: "Discord", href: "#" } ] } ], copyright: "© Docs" , year: new Date().getFullYear() } }
//       ],
//     },
//   },
// } as const;
...
//       theme: "light",
//     },
//     render: ({ children, title, description, viewport, theme }: any) => {
//       return (
//         <div
//           data-theme={theme}
//           style={{
...
//   }: any) => {
//     const parentType =
//       puck?.parentProps?.type || puck?.parent?.type || puck?.parentType;

//     const isInGrid =
//       parentType === "Grid" || parentType === "ResponsiveGrid";
//     const isInFlex =
