...
import PuckRenderWrapper from "@/app/published/PuckRenderWrapper";
...
function PuckEditor() {
  const ReactJson: any = useMemo(() => dynamic(() => import('react-json-view'), { ssr: false }) as any, []);
  const [previewTab, setPreviewTab] = useState<'ui' | 'json' | 'lite'>('ui');
  class PreviewErrorBoundary extends React.Component<{ children: React.ReactNode }, { hasError: boolean; msg?: string }> {
  ...
  const getGroupId = (group: any) => String(group?._id || group?.id || "");
  ...
  // Default preview group to first pending invite when modal opened in pending mode
  useEffect(() => {
    if (manageGroupsMode !== 'pending') {
      setPreviewGroup(null);
      return;
    }
    setPreviewGroup((current) => {
      if (current && pendingSharedGroups.some((g: any) => g?._id === current?._id)) {
        return current;
      }
      return null;
    });
  }, [manageGroupsMode, pendingSharedGroups]);

  // Lazy-load preview trees for shared groups when needed
  useEffect(() => {
    if (!previewGroup) return;
    const id = getGroupId(previewGroup);
    if (!id) return;
    const inlineTree = previewGroup.tree || previewGroup.previewTree || previewGroup.snapshot;
    if (inlineTree || previewCache[id]) {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/groups/${id}?preview=1`, { cache: 'no-store' });
        if (!res.ok) throw new Error('Prévisualisation indisponible');
        const json = await res.json();
        const tree = json?.group?.tree || json?.tree || json?.previewTree || null;
        if (!cancelled) {
          if (tree) {
            setPreviewCache((prev) => ({ ...prev, [id]: tree }));
            setPreviewError(null);
          } else {
            setPreviewError('Aucun contenu fourni pour ce groupe.');
          }
        }
      } catch (err: any) {
        if (!cancelled) {
  ...
  const previewData = useMemo(() => {
    ...
    const direct = previewGroup.tree || previewGroup.previewTree || previewGroup.snapshot || null;
    const cached = previewCache[getGroupId(previewGroup)];
    const source = direct || cached || null;
    // console.log('[PreviewDebug] Building previewData:', { groupId: getGroupId(previewGroup), hasDirect: !!direct, hasCached: !!cached, source });
    if (!source) return null;
    if (source.root && Array.isArray(source.root.content)) {
      // console.log('[PreviewDebug] Using source.root.content, items:', source.root.content.length);
      return source;
    }
    if (Array.isArray(source)) {
      // console.log('[PreviewDebug] Source is array, wrapping in root, items:', source.length);
      return { root: { content: source } };
    }
    if (source.content && Array.isArray(source.content)) {
      // console.log('[PreviewDebug] Using source.content, items:', source.content.length);
      return { root: { content: source.content } };
    }
    // console.log('[PreviewDebug] No valid structure found');
    return null;
  }, [previewGroup, previewCache]);

  // Create a preview-safe version of the data that strips or flattens
  // any nodes whose component types are not registered in mergedConfig.
  const previewUiData = useMemo(() => {
    if (!previewData) return null as any;
    try {
      const known = new Set<string>(Object.keys((mergedConfig as any)?.components || {}));
      const cleanseArray = (arr: any[]): any[] => {
        const out: any[] = [];
        for (const n of arr || []) {
          ...
          if (t && known.has(t)) {
            ...
            // Fix: If props.content exists but top-level content doesn't, hoist it
            if (Array.isArray((n as any).props?.content) && !Array.isArray((n as any).content)) {
              next.content = cleanseArray((n as any).props.content);
              // Remove from props to avoid duplication
              if (next.props) {
                const { content: _, ...restProps } = next.props;
                next.props = restProps;
              }
            } else if (Array.isArray((n as any).content)) {
              next.content = cleanseArray((n as any).content);
            }
            
            // Fix: If props.children exists and is an array, cleanse it
            if (next.props && Array.isArray((n as any).props?.children)) {
              next.props = {
                ...next.props,
                children: cleanseArray((n as any).props.children)
              };
            }

            if ((n as any).slots && typeof (n as any).slots === 'object') {
              const slots: any = {};
              for (const [k, v] of Object.entries((n as any).slots)) {
                slots[k] = Array.isArray(v) ? cleanseArray(v as any) : v;
              }
              next.slots = slots;
            }
            if ((n as any).zones && typeof (n as any).zones === 'object') {
              const zones: any = {};
              for (const [k, v] of Object.entries((n as any).zones)) {
                zones[k] = Array.isArray(v) ? cleanseArray(v as any) : v;
              }
              next.zones = zones;
            }
            if (Array.isArray((n as any).children) && !next.props?.children) {
              // Only hoist to content if there's no props.children
              next.content = cleanseArray((n as any).children);
            }
            out.push(next);
          } else if (Array.isArray((n as any).content)) {
            // Unknown component: hoist its children into parent so something renders
            out.push(...cleanseArray((n as any).content));
          } else if (Array.isArray((n as any).props?.content)) {
            // Also check props.content for unknown types
            out.push(...cleanseArray((n as any).props.content));
          } else if (Array.isArray((n as any).children)) {
            out.push(...cleanseArray((n as any).children));
          } else if ((n as any).slots && typeof (n as any).slots === 'object') {
            for (const v of Object.values((n as any).slots)) {
              if (Array.isArray(v)) out.push(...cleanseArray(v as any));
            }
          } else if ((n as any).zones && typeof (n as any).zones === 'object') {
            for (const v of Object.values((n as any).zones)) {
              if (Array.isArray(v)) out.push(...cleanseArray(v as any));
            }
          } else {
            // Drop unknown leaf nodes
          }
        }
        return out;
      };
      const content = Array.isArray((previewData as any)?.root?.content)
        ? cleanseArray((previewData as any).root.content)
        : [];
      return { root: { content } };
    } catch {
      return previewData;
    }
  }, [previewData, mergedConfig]);
  ...
  const renderPendingInvitationsSection = () => {
    if (pendingSharedGroups.length === 0) {
      return (
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)]">
        <div className="space-y-3">
          {pendingSharedGroups.map((g: any) => {
            ...
            const isActive = previewGroup ? getGroupId(previewGroup) === groupId : false;
            const handleSelect = () => setPreviewGroup(g);
            ...
            return (
              <div
                ...
                onKeyDown={handleKey}
                className={`rounded-lg border border-amber-200 bg-amber-50 p-3 transition ring-offset-2 focus:outline-none ${isActive ? 'ring-2 ring-amber-400 bg-white shadow-sm' : ''
                  }`}
              >
        ...
        <div className="rounded-lg border border-gray-200 bg-white/90 p-3 shadow-inner">
          <div className="text-xs font-semibold uppercase tracking-wide text-gray-500">Aperçu en direct</div>
          <div className="flex items-center justify-between mt-1">
            <p className="text-[11px] text-gray-500">Cliquez sur un groupe partagé pour visualiser son contenu.</p>
            <div className="inline-flex rounded-md border border-gray-200 overflow-hidden">
              <button
                type="button"
                onClick={() => setPreviewTab('ui')}
                className={`px-2 py-[2px] text-[11px] ${previewTab === 'ui' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700'}`}
                title="Afficher le rendu UI"
              >UI</button>
              <button
                type="button"
                onClick={() => setPreviewTab('json')}
                className={`px-2 py-[2px] text-[11px] ${previewTab === 'json' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700'}`}
                title="Afficher le JSON"
              >JSON</button>
              <button
                type="button"
                onClick={() => setPreviewTab('lite')}
                className={`px-2 py-[2px] text-[11px] ${previewTab === 'lite' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700'}`}
                title="Rendu générique (compatibilité maximale)"
              >Lite</button>
            </div>
          </div>
          <div className="mt-3 rounded-md border border-dashed border-gray-200 bg-gray-50 p-2 h-[300px] overflow-auto">
            {previewLoadingId && previewGroup && getGroupId(previewGroup) === previewLoadingId ? (
              <div className="flex h-full items-center justify-center text-xs text-gray-500">Chargement de l’aperçu…</div>
            ) : previewData ? (
              previewTab === 'ui' ? (
                <div className="bg-white rounded-md shadow-sm p-2">
                  <PreviewErrorBoundary>
                    {(() => {
                      const items = Array.isArray((previewData as any)?.root?.content) ? (previewData as any).root.content.length : 0;
                      console.log('[PreviewDebug] UI tab rendering, items:', items);
                      console.log('[PreviewDebug] previewData:', previewData);
                      console.log('[PreviewDebug] previewUiData:', previewUiData);
                      console.log('[PreviewDebug] previewUiData.root.content[0]:', (previewUiData as any)?.root?.content?.[0]);
                      console.log('[PreviewDebug] mergedConfig components:', Object.keys((mergedConfig as any)?.components || {}));
                      if (items === 0) {
                        return <div className="text-xs text-gray-500 text-center py-8">Ce groupe ne contient aucun élément pour le moment.</div>;
                      }


                      return (
                        <ActionStateProvider allowCustomJS={true}>
                          <Render
                            config={mergedConfig as any}
                            data={previewUiData || previewData}
                          />
                        </ActionStateProvider>
                      );
                    })()}
                ...
                <div className="bg-white rounded-md shadow-sm p-2">
                  <SimplePreview data={previewData} />
                </div>
              ) : (
                <div className="bg-white rounded-md shadow-sm p-2">
                  {ReactJson ? (
                    <ReactJson src={previewData} name={false} collapsed={2} enableClipboard={false} displayDataTypes={false} />
                  ) : (
                    <pre className="text-[11px] text-gray-700 whitespace-pre-wrap">{JSON.stringify(previewData, null, 2)}</pre>
                  )}
                </div>
              )
            ) : (
              <div className="flex h-full items-center justify-center px-4 text-center text-xs text-gray-400">
                {previewError || 'Sélectionnez un groupe pour afficher son contenu.'}
              </div>
            )}
          </div>
          {previewGroup && (
            <p className="mt-2 text-[11px] text-gray-500">
              Aperçu : <span className="font-medium text-gray-700">{previewGroup.name || 'Groupe partagé'}</span>
              {previewGroup.ownerEmail ? ` · partagé par ${previewGroup.ownerEmail}` : ''}
            </p>
          )}
          {previewError && (
            <p className="mt-1 text-[11px] text-amber-700">{previewError}</p>
          )}
        </div>
      </div>
    );
  ...
  const renderOwnedGroupsSection = () => (
    <>
      <div>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2 text-sm font-semibold text-gray-900">
                      {g.name || 'Sans titre'}
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${g.public ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'
                          }`}
                      >
  ...
  const renderManageGroupsBody = () => (
    <div className="max-h-[70vh] overflow-y-auto px-5 py-4 space-y-4">
      {manageGroupsMode === 'pending' ? renderPendingInvitationsSection() : renderOwnedGroupsSection()}
    </div>
  );
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <div className="flex items-center gap-1 px-2 py-1 rounded border border-dashed border-gray-300 bg-white text-[10px] text-gray-600">
                            ...
                            <button
                              ...
                              onClick={() => {
                                try {
                                  if (lastPuckPathRef.current) {
                                    selectionStore.set([lastPuckPathRef.current]);
                                    // console.log('[PuckDebug] Force selected last path:', lastPuckPathRef.current);
                                  }
                                } catch { }
                              }}
                                ...
                                groups.map((g: any) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <span
                                        className={`ml-2 inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${g.public
                    ...
                    <div className="mt-4 p-3 border border-gray-200 rounded-md bg-gray-50">
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{
                          __html: buildPreviewFromOptions({
                            label: optLabel,
                            href: optHref,
                            theme: optTheme,
                            variant: optVariant,
