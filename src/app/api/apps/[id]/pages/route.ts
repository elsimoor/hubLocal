...
export async function GET(
  ...
  // In Next.js 15 dynamic route handlers, params is provided as a Promise.
...
export async function POST(
  ...
  if (existing)
    return NextResponse.json({ error: "page exists" }, { status: 409 });
  const created = await PuckDocModel.create({
    ownerEmail: session.user.email,
    slug,
    status: "draft",
    data: { root: { props: { title: pageSlug } }, content: [] },
  });
