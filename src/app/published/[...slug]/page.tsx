...
// With Next.js 15, params/searchParams are Promises in RSC. Unwrap them.
export default async function PublishedPage({ params, searchParams }: { params: Promise<{ slug: string[] }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  try {
    ...
    const url = `${base}/api/puck/published/${(slug || []).map(encodeURIComponent).join("/")}${debugOn ? "?debug=1" : ""}`;
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }): Promise<Metadata> {
  try {
    ...
    const apiUrl = `${base}/api/puck/published/${slugPath}`;
    ...
    const canonical = typeof rp.metaCanonical === 'string' && rp.metaCanonical.trim() ? rp.metaCanonical : `${base}/published/${slugPath}`;
    ...
    return {
      title,
      description,
      keywords: keywords ? keywords.split(',').map((k:any) => k.trim()).filter(Boolean) : undefined,
